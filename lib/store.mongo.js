...
module.exports = function (DEFAULT_SETTINGS) {
  ...
  const dbName = process.env.MONGODB_DB || 'anhanh_biolink';
  ...
  async function getSettings() {
    ...
    const { _id, avatarFileId, songFileId, ...rest } = doc;
    return rest;
  }
  ...
  async function saveUpload(kind, file) {
    ...
    const oldFieldId = kind === 'avatar' ? current.avatarFileId : current.songFileId;
    ...
    const fieldIdKey = kind === 'avatar' ? 'avatarFileId' : 'songFileId';
    await col.updateOne(
      { _id: SETTINGS_ID },
