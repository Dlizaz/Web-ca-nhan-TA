...
app.post('/api/settings', requireAuth, async (req, res) => {
  try {
    ...
    // Code cấu hình JSON (tsParticles) tự dán, riêng cho từng loại kim tuyến (con trỏ / quanh tên / rải khắp trang).
    // Chỉ chấp nhận JSON hợp lệ (dữ liệu thuần, không phải code thực thi) và giới hạn độ dài để tránh phình dữ liệu.
    const applyParticlesConfigField = (key) => {
      if (typeof b[key] !== 'string') return;
      const trimmed = b[key].trim();
      if (!trimmed) {
        patch[key] = '';
    ...
    applyParticlesConfigField('sparkleCursorConfig');
    applyParticlesConfigField('usernameSparkleConfig');
    applyParticlesConfigField('pageParticlesConfig');
