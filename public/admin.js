...
async function loadSettings() {
  ...
  document.getElementById('field-sparkleCursorConfig').value = settings.sparkleCursorConfig || '';
  document.getElementById('field-usernameSparkleConfig').value = settings.usernameSparkleConfig || '';
...
// ---------- Hiệu ứng kim tuyến: 3 loại riêng biệt (con trỏ / tên / cả trang), mỗi loại có ô dán code riêng ----------
document.getElementById('save-sparkle').addEventListener('click', () => {
  ...
  const sparkleCursorConfig = document.getElementById('field-sparkleCursorConfig').value.trim();
  const usernameSparkleConfig = document.getElementById('field-usernameSparkleConfig').value.trim();
  ...
  const configsToCheck = [
    ['con trỏ chuột', sparkleCursorConfig],
    ['quanh tên', usernameSparkleConfig],
    ['rải khắp trang', pageParticlesConfig],
  ];
  for (const [label, raw] of configsToCheck) {
    if (!raw) continue;
    try {
      JSON.parse(raw);
    } catch (e) {
      statusEl.textContent = `Code JSON ở ô "${label}" không hợp lệ, kiểm tra lại dấu ngoặc/dấu phẩy nhé.`;
  ...
  saveSettings({
    sparkleCursorColor, usernameSparkleColor, sparklePageColor,
    sparkleCursorConfig, usernameSparkleConfig, pageParticlesConfig,
  }, 'status-sparkle');
});
