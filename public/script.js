...
function startCustomUsernameEffect(effectConfig={}) {
  ...
  // IMPORTANT: the effect is anchored to the actual text span, not the H1,
  // profile card, bio, or viewport.
  const textEl=document.getElementById('username-text');
  const container=document.getElementById('tsparticles-username');
  if(!textEl || !container || effectConfig.enable===false) return;

  // Move the effect container INSIDE the exact text element.
  // This makes its containing block the username itself.
  if(container.parentElement !== textEl) textEl.appendChild(container);
  ...
  const sizeRange=randomRange(effectConfig.size??universal.size,0.8,1.8);
  const opacityRange=randomRange(effectConfig.opacity??universal.opacity,.08,.85);
  const count=Math.max(1,Math.round(Number(effectConfig.count??45)));
  const speedRange=randomRange(effectConfig.speed??universal.move?.speed,.45,1.15);
  const fadeRange=randomRange(effectConfig.fadeTime??universal.fade?.time,1800,3200);
  const visibleRange=randomRange(effectConfig.visibleTime??universal.timing?.visible,1200,3000);
  const hiddenRange=randomRange(effectConfig.hiddenTime??universal.timing?.hidden,700,1800);
  const rot=effectConfig.rotation??universal.rotation??{};
  const rotSpeed=randomRange(rot.speed,.05,.25);
  const rotEnabled=rot.enable!==false;

  const area=effectConfig.area&&typeof effectConfig.area==='object'?effectConfig.area:{};
  const padX=Number.isFinite(Number(area.paddingX))?Number(area.paddingX):10;
  const padY=Number.isFinite(Number(area.paddingY))?Number(area.paddingY):8;

  const canvas=document.createElement('canvas');
  canvas.className='custom-username-effect-canvas';
  container.innerHTML='';
  ...
  // Canvas and particle coordinates are LOCAL to #username-text.
  function resize(){
    ...
    canvas.width=Math.max(1,Math.ceil((width+padX*2)*dpr));
    canvas.height=Math.max(1,Math.ceil((height+padY*2)*dpr));
    canvas.style.width=(width+padX*2)+'px';
    canvas.style.height=(height+padY*2)+'px';
    canvas.style.left=(-padX)+'px';
    canvas.style.top=(-padY)+'px';
  ...
  function phase(){
    const f=randomBetween(fadeRange),q=Math.random();
    if(q<.28)return {state:'hidden',timer:randomBetween(hiddenRange),fade:f};
    if(q<.52)return {state:'fadeIn',timer:f,fade:f};
    if(q<.82)return {state:'visible',timer:randomBetween(visibleRange),fade:f};
    return {state:'fadeOut',timer:f,fade:f};
  }

  // Spawn only in a thin ring around the username, never across the bio/profile.
  function spawn(){
    const side=Math.floor(Math.random()*4);
    let x,y;
    if(side===0){x=Math.random()*width;y=-Math.random()*padY;}
    else if(side===1){x=width+Math.random()*padX;y=Math.random()*height;}
    else if(side===2){x=Math.random()*width;y=height+Math.random()*padY;}
    else{x=-Math.random()*padX;y=Math.random()*height;}

    const ph=phase();
    particles.push({
      x,y,baseX:x,baseY:y,
      drift:Math.random()*Math.PI*2,
      driftSpeed:randomBetween(speedRange),
      driftRadius:.8+Math.random()*2.2,
      size:randomBetween(sizeRange),
      color:colors[Math.floor(Math.random()*colors.length)],
      opacity:0,
      targetOpacity:randomBetween(opacityRange),
      state:ph.state,timer:ph.timer,fade:ph.fade,
      rotation:Math.random()*Math.PI*2,
      rotationDirection:Math.random()<.5?-1:1,
      rotationSpeed:randomBetween(rotSpeed)
    });
  }
  ...
  function update(p,dt){
    ...
    if(p.state==='hidden'){
      p.opacity=0;
      if(p.timer<=0){p.state='fadeIn';p.fade=randomBetween(fadeRange);p.timer=p.fade;}
    }else if(p.state==='fadeIn'){
      const q=Math.max(0,Math.min(1,1-p.timer/p.fade));
      const e=q*q*(3-2*q);
      p.opacity=p.targetOpacity*e;
      if(p.timer<=0){p.state='visible';p.timer=randomBetween(visibleRange);p.opacity=p.targetOpacity;}
    }else if(p.state==='visible'){
      p.opacity=p.targetOpacity;
      if(p.timer<=0){p.state='fadeOut';p.fade=randomBetween(fadeRange);p.timer=p.fade;}
    }else{
      const q=Math.max(0,Math.min(1,1-p.timer/p.fade));
      const e=q*q*(3-2*q);
      p.opacity=p.targetOpacity*(1-e);
      if(p.timer<=0){
        p.state='hidden';
        p.timer=randomBetween(hiddenRange);
        p.opacity=0;
        p.targetOpacity=randomBetween(opacityRange);
        p.color=colors[Math.floor(Math.random()*colors.length)];
        const side=Math.floor(Math.random()*4);
        if(side===0){p.x=Math.random()*width;p.y=-Math.random()*padY;}
        else if(side===1){p.x=width+Math.random()*padX;p.y=Math.random()*height;}
        else if(side===2){p.x=Math.random()*width;p.y=height+Math.random()*padY;}
        else{p.x=-Math.random()*padX;p.y=Math.random()*height;}
        p.baseX=p.x;p.baseY=p.y;
      }
    }

    p.drift+=p.driftSpeed*dt*.001;
    p.x=p.baseX+Math.cos(p.drift)*p.driftRadius;
    p.y=p.baseY+Math.sin(p.drift*.83)*p.driftRadius;
    if(rotEnabled)p.rotation+=p.rotationDirection*p.rotationSpeed*dt*.001;
  }

  let last=performance.now();
  const state={canvas,resizeObserver,resizeHandler:resize,raf:0};
  customUsernameAnimations.set('tsparticles-username',state);

  function frame(now){
    if(!customUsernameAnimations.has('tsparticles-username'))return;
    const dt=Math.min(40,now-last);last=now;
    ctx.clearRect(-padX,-padY,width+padX*2,height+padY*2);
    for(const p of particles){
      update(p,dt);
      if(p.opacity>.001)drawUsernameSparkle(ctx,p.x,p.y,p.size,p.color,p.opacity,p.rotation);
    }
