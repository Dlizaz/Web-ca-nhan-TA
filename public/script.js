...
// ---------- Hiệu ứng "kim tuyến rải khắp trang" bằng code JSON tự dán (thư viện tsParticles) ----------
// 'default' = dùng hiệu ứng particle tự chế (canvas #fx-canvas, màu sparklePageColor)
// 'custom'  = dùng đúng code JSON admin dán, chạy qua thư viện tsParticles trong #tsparticles-page
...
async function applyPageParticlesConfig() {
  const raw = (siteSettings && siteSettings.pageParticlesConfig || '').trim();

  if (!raw) {
    pageEffectMode = 'default';
    if (window.tsParticles && typeof window.tsParticles.dom === 'function') {
      const existing = window.tsParticles.dom().find((c) => c.id === 'tsparticles-page');
  ...
  try {
    ...
    await window.tsParticles.load({ id: 'tsparticles-page', options: config });
    pageEffectMode = 'custom';
  } catch (err) {
    // Code lỗi hoặc không tải được thư viện -> lặng lẽ quay về hiệu ứng tự chế, không làm hỏng cả trang
    console.warn('Cấu hình kim tuyến rải khắp trang không dùng được, quay về hiệu ứng mặc định:', err);
    pageEffectMode = 'default';
  }
}
