...
// ---------- Hiệu ứng kim tuyến bằng code JSON tự dán (thư viện tsParticles), riêng cho từng loại ----------
// Với mỗi loại: 'default' = dùng hiệu ứng tự chế (particle canvas / sao quanh tên) + màu đã chọn
//               'custom'  = dùng đúng code JSON admin dán, chạy qua thư viện tsParticles trong container riêng
let cursorEffectMode = 'default';   // #tsparticles-cursor   (kim tuyến theo con trỏ chuột)
let usernameEffectMode = 'default'; // #tsparticles-username (kim tuyến quanh tên)
let pageEffectMode = 'default';     // #tsparticles-page     (kim tuyến rải khắp trang)
...
// Nạp/gỡ 1 hiệu ứng kim tuyến tùy chỉnh vào đúng container của nó.
// setMode(...) được gọi ngay khi biết code JSON hợp lệ hay không (không cần đợi thư viện tải xong),
// để các hiệu ứng mặc định (particle canvas / sao quanh tên) biết đường nhường chỗ kịp lúc.
async function applyCustomParticles(containerId, configRaw, setMode) {
  ...
  try {
    ...
    await window.tsParticles.load({ id: containerId, options: config });
  } catch (err) {
    // Code lỗi hoặc không tải được thư viện -> lặng lẽ quay về hiệu ứng mặc định, không làm hỏng cả trang
